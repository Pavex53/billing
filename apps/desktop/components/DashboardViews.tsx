...
export const DashboardHome: React.FC<ViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
      ...
      {/* 1. White Card - Revenue / Bestsellers */}
      <div className="bg-white rounded-[2.5rem] p-8 text-black relative overflow-hidden min-h-[420px] flex flex-col shadow-sm animate-enter premium-hover">
      ...
      {/* 2. Info Card - Taxes (Umsatzsteuer) */}
      <div className="bg-info rounded-[2.5rem] p-6 text-black min-h-[420px] flex flex-col shadow-sm relative overflow-hidden animate-enter delay-100 premium-hover">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm overflow-hidden p-8 min-h-full flex flex-col relative animate-enter">
                        ...
                        <div className="flex flex-wrap gap-3 items-end">
                          <div className="flex-1 min-w-[260px]">
                            ...
                            <div className="flex gap-2">
                              <input value={importPath ?? ''} readOnly className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-mono" />
                              <button disabled={isImportBusy} onClick={openImportWithPicker} className="px-4 py-2 rounded-xl font-bold bg-white border border-gray-200 hover:bg-gray-50 transition-colors text-sm">Ändern</button>
                            </div>
                          </div>
                          <div>
                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Profil</label>
                            <select value={importProfile} onChange={async (e) => { const p = e.target.value as ImportProfile; setImportProfile(p); setImportMapping(null); await refreshImportPreview({ profile: p, mapping: null }); }} className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold">
                          ...
                          <div>
                            ...
                            <select value={importEncoding} onChange={async (e) => { const enc = e.target.value as 'utf8' | 'win1252'; setImportEncoding(enc); await refreshImportPreview({}); }} className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold">
                          ...
                          <div>
                            ...
                            <input value={importDelimiter} onChange={(e) => setImportDelimiter(e.target.value)} placeholder="(auto)" className="w-28 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-mono" />
                          </div>
                          <button disabled={isImportBusy || !importPath} onClick={() => refreshImportPreview({})} className="px-4 py-2 rounded-xl font-bold bg-black text-white hover:bg-gray-800 transition-colors text-sm">Vorschau</button>
                        </div>
                          ...
                          <>
                            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <select value={importMapping.dateColumn} onChange={(e) => setImportMapping({ ...importMapping, dateColumn: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold">
                              ...
                              <div>
                                ...
                                <select value={importMapping.amountColumn} onChange={(e) => setImportMapping({ ...importMapping, amountColumn: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold">
                                  {importPreview.headers.map((h: string) => <option key={h} value={h}>{h}</option>)}
                                </select>
                              </div>
                              <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Gegenpartei</label>
                                <select value={importMapping.counterpartyColumn ?? ''} onChange={(e) => setImportMapping({ ...importMapping, counterpartyColumn: e.target.value || undefined })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold">
                              ...
                              <div>
                                ...
                                <select value={importMapping.purposeColumn ?? ''} onChange={(e) => setImportMapping({ ...importMapping, purposeColumn: e.target.value || undefined })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold">
                                  <option value="">(leer)</option>
                                  {importPreview.headers.map((h: string) => <option key={h} value={h}>{h}</option>)}
                                </select>
                              </div>
                            ...
                            <div className="mt-6 flex items-center justify-between">
                              <div className="text-sm text-gray-600">
                                <span className="font-bold">{importPreview.stats.validRows}</span> ok · <span className="font-bold">{importPreview.stats.errorRows}</span> Fehler · <span className="font-bold">{importPreview.stats.totalRows}</span> Zeilen
                              </div>
                              <button disabled={isImportBusy || !importMapping} onClick={commitImport} className="px-5 py-3 rounded-2xl font-bold bg-accent text-black hover:bg-accent-hover transition-colors">Import starten</button>
                            </div>
                            {importError && <div className="mt-4 p-4 rounded-2xl border border-error/30 bg-error-bg text-error font-mono text-xs">{importError}</div>}
                            {importResult && (
                            ...
                            <div className="mt-6">
                              <div className="flex items-center justify-between mb-2">
                                ...
                                {importPreview.stats.errorRows > 0 && (
                                  <button onClick={() => setShowOnlyErrors(!showOnlyErrors)} className="px-3 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-xs font-bold transition-colors">
                              ...
                              <div className="border border-gray-100 rounded-2xl overflow-hidden">
                                <table className="w-full text-sm">
                                  ...
                                  <tbody>
                                    {(() => {
                                      const filteredRows = showOnlyErrors ? importPreview.rows.filter((r: any) => r.errors.length > 0) : importPreview.rows;
                              ...
                              {(() => {
                                ...
                                return (
                                  <div className="mt-3 flex items-center justify-between text-sm">
                                    <div className="text-gray-600">Seite {importPreviewPage + 1} von {totalPages} ({filteredRows.length} Zeilen)</div>
                                    <div className="flex gap-2">
                                      <button onClick={() => setImportPreviewPage(Math.max(0, importPreviewPage - 1))} disabled={importPreviewPage === 0} className="px-3 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed font-bold transition-colors">Zurück</button>
                                      <button onClick={() => setImportPreviewPage(Math.min(totalPages - 1, importPreviewPage + 1))} disabled={importPreviewPage >= totalPages - 1} className="px-3 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed font-bold transition-colors">Weiter</button>
                                    </div>
                                  </div>
                                );
                              })()}
                ...
                {isLinkModalOpen && (
                    <div className="absolute inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 rounded-[2.5rem] animate-fade-in">
                        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[600px] animate-scale-in">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                                <h3 className="font-bold text-lg flex items-center gap-2"><Link size={18} />Transaktion zuweisen</h3>
                            ...
                            <div className="p-4 border-b border-gray-100">
                                <div className="relative">
                                    ...
                                    <input type="text" placeholder="Rechnung oder Kunde suchen..." value={invoiceSearch} onChange={(e) => setInvoiceSearch(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm font-medium focus:ring-2 focus:ring-accent outline-none transition-shadow" />
                                </div>
                            </div>
                            <div className="overflow-y-auto p-4 space-y-2">
                                {filteredInvoices.length > 0 ? filteredInvoices.map(inv => (
                                    <div key={inv.id} onClick={() => handleConfirmLink(inv.id)} className="p-4 rounded-xl border border-gray-100 hover:border-black hover:bg-gray-50 cursor-pointer transition-all group">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className={`text-[10px] font-bold px-2 py-1 rounded ${inv.status === 'overdue' ? 'bg-error-bg text-error' : 'bg-success-bg text-success'}`}>{inv.status.toUpperCase()}</span>
                                        </div>
                                    </div>
                                )) : (
                ...
                <div className="flex items-center justify-between mb-8 pb-8 border-b border-gray-100">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setSelectedAccountId(null)} className="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center hover:bg-black hover:text-white transition-colors">
                            <ArrowLeft size={18} />
                        </button>
                        <div>
                    ...
                    <div className="flex items-end gap-3">
                        <button onClick={openImportWithPicker} className="h-10 px-4 bg-black text-white rounded-full text-xs font-bold hover:bg-gray-800 transition-colors flex items-center gap-2">CSV importieren</button>
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    <div className="space-y-3">
                        {selectedAccount.transactions.map((tx, i) => {
                            const linkedInvoice = tx.linkedInvoiceId ? invoices.find(i => i.id === tx.linkedInvoiceId) : null;
                            return (
                                <div key={tx.id} className="group flex items-center justify-between p-4 rounded-2xl border border-gray-100 hover:border-gray-300 hover:shadow-md transition-all bg-white animate-enter" style={{ animationDelay: `${i * 50}ms` }}>
                                    <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-6">
                                        <div className="flex flex-col items-end gap-1">
                                            ...
                                            {linkedInvoice ? (
                                                ...
                                                tx.type === 'income' && (
                                                    <button onClick={() => handleLinkClick(tx.id)} className="flex items-center gap-1.5 bg-black text-accent px-3 py-1 rounded text-[10px] font-bold hover:bg-gray-800 transition-colors opacity-0 group-hover:opacity-100 transform translate-y-1 group-hover:translate-y-0 duration-200">
                                                        <Link size={10} />
                                                        Zuweisen
                                                    </button>
                                                )
    ...
    return (
        <div className="bg-white rounded-[2.5rem] shadow-sm overflow-hidden p-8 h-full animate-enter">
            <div className="flex items-center justify-between mb-8">
                ...
                <button
                  onClick={() => {
                    setNewAccount({ id: uuidv4(), name: '', iban: '', balance: 0, type: 'bank', color: 'bg-white', transactions: [] });
                ...
                <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
                  ...
                  <div className="p-6 space-y-4">
                    <div>
                      ...
                      <input value={newAccount.name} onChange={(e) => setNewAccount({ ...newAccount, name: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-accent" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Typ</label>
                        <select value={newAccount.type} onChange={(e) => setNewAccount({ ...newAccount, type: e.target.value as any })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm font-bold">
                      ...
                      <div>
                        ...
                        <input type="number" value={newAccount.balance} onChange={(e) => setNewAccount({ ...newAccount, balance: Number(e.target.value) })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm font-mono font-bold" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">IBAN / Identifier</label>
                      <input value={newAccount.iban} onChange={(e) => setNewAccount({ ...newAccount, iban: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-mono outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Farbe</label>
                      <select value={newAccount.color} onChange={(e) => setNewAccount({ ...newAccount, color: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm font-bold">
                  ...
                  <div className="p-6 border-t border-gray-100 flex justify-end gap-2 bg-white">
                    <button onClick={() => setIsAddAccountOpen(false)} className="px-5 py-2.5 rounded-xl font-bold bg-gray-100 hover:bg-gray-200 transition-colors">Abbrechen</button>
                    <button onClick={() => { if (!newAccount.name.trim()) return; upsertAccount.mutate(newAccount, { onSuccess: () => setIsAddAccountOpen(false) }); }} className="px-5 py-2.5 rounded-xl font-bold bg-black text-white hover:bg-gray-800 transition-colors">Speichern</button>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-4">
                {accounts.map((acc, i) => (
                    <div key={i} onClick={() => setSelectedAccountId(acc.id)} className={`p-6 rounded-3xl ${acc.color} flex items-center justify-between cursor-pointer border border-transparent hover:border-black/10 hover:shadow-xl hover:-translate-y-1 transition-all group animate-enter`} style={{ animationDelay: `${i * 100}ms` }}>
                        <div className="flex items-center gap-4">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm p-8 min-h-[80vh] animate-enter">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="bg-gray-100 p-1 rounded-full flex items-center">
                    <button onClick={() => setActiveTab('invoice')} className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${activeTab === 'invoice' ? 'bg-white shadow text-black' : 'text-gray-400 hover:text-gray-600'}`}>Rechnungen</button>
                    <button onClick={() => setActiveTab('offer')} className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${activeTab === 'offer' ? 'bg-white shadow text-black' : 'text-gray-400 hover:text-gray-600'}`}>Angebote</button>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div onClick={() => void handleCreateNewTemplate()} className="aspect-[3/4] bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-4 cursor-pointer hover:border-black hover:bg-gray-100 transition-all group animate-scale-in">
                ...
                {templates.map((t, idx) => (
                <div key={t.id} className="aspect-[3/4] bg-surface rounded-[2rem] border border-border p-4 flex flex-col hover:border-border hover:-translate-y-2 transition-all cursor-pointer group relative overflow-hidden animate-scale-in" style={{ animationDelay: `${100 + idx * 50}ms` }} onClick={() => onOpenEditor(activeTab)}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); void setActiveTemplateMutation.mutateAsync({ kind: activeTab, templateId: t.id }); }} className={`absolute top-4 left-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase border transition-colors ${ t.id === activeTemplate?.id ? 'bg-accent text-black border-accent' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-100' }`}>
